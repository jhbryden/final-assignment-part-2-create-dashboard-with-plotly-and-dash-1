...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        # use groupby to create relevant data for plotting
        unemp_data= recession_data.groupby('Vehicle_Type')['Unemployment Rate'].sum().reset_index()
        R_chart4 = dcc.Graph(figure=px.bar(exp_rec, 
        x='Unemployment Rate',
        y='Vehicle_Type',
        title='Unemployment Rate on Vehicle Type and Sales during the Recession'))
...
# use groupby to create relevant data for plotting
        yearly_rec=recession_data.groupby('Year')['Automobile Sales'].mean().reset_index()
        Y_chart1 = dcc.Graph(figure=px.line(yearly_rec, 
        x='Year',
        y='Automobile Sales',
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        # use groupby to create relevant data for plotting
        average_sales = recession_data.groupby('Vehicle_Type')['Automobile_Sales'].mean().reset_index()                           
        Y_chart2  = dcc.Graph(figure=px.line(average_sales,
        x='Vehicle_Type',
        y='Automobile_Sales',
        title='Average Automobile Sales for each Vehicle Type in the Recession Period {}'.format(input_year)))
        
# Plot 3 Pie chart for total expenditure share by vehicle type during recessions
        # use groupby to create relevant data for plotting
        exp_rec= recession_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum().reset_index()
        Y_chart3 = dcc.Graph(figure=px.pie(exp_rec, 
        values='Advertising_Expenditure',
        names='Vehicle_Type',
        title='Total Expenditure for each Vehicle Type during the Recession {}'.format(input_year)))

# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        # use groupby to create relevant data for plotting
        exp_rec= recession_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum().reset_index()
        Y_chart4 = dcc.Graph(figure=px.bar(exp_rec, 
        x='Advertising_Expenditure',
        y='Vehicle_Type',
        title='Effect of Unemployment Rate for each Vehicle Type during the Recession {}'.format(input_year)))
